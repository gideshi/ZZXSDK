...
namespace ZZX.Parser
{
    ...
    public class ZZXJsonReader : IZZXReader
    {
        private IDictionary json;

        public ZZXJsonReader(IDictionary json)
        {
            this.json = json;
        }
        ...
        public object GetReferenceObject(object name, Type type, DZZXConvert convert)
        {
            IDictionary dict = json[name] as IDictionary;
            if (dict != null && dict.Count > 0)
            {
                return convert(new ZZXJsonReader(dict), type);
            }
            else
            {
                return null;
        ...
        public IList GetListObjects(string listName, string itemName, Type type, DZZXConvert convert)
        {
            IList listObjs = null;
            ...
            if (jsonList != null && jsonList.Count > 0)
            {
                ...
                foreach (object item in jsonList)
                {
                    if (typeof(IDictionary).IsAssignableFrom(item.GetType())) // object
                    {
                        ...
                        object subObj = convert(new ZZXJsonReader(subMap), type);
                        if (subObj != null)
                        {
                            listObjs.Add(subObj);
                        }
