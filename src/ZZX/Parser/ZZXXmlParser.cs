...
namespace ZZX.Parser
{
    ...
    /// ZMOP XML响应通用解释器。
    /// </summary>
    public class ZZXXmlParser<T> : IZZXParser<T> where T : ZZXResponse
    {
        ...
        private string GetRootElement(string body)
        {
            ...
            {
                throw new ZZXException("Invalid XML response format!");
            }
        }

        private static string GetSign(string body)
        {
            string signNodeName = "<" + ZZXConstants.SIGN + ">";
            string signEndNodeName = "</" + ZZXConstants.SIGN + ">";
